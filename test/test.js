...
const LambdaLayerGit = require('../index').prepareGit;

const BIN_PATH = 'git';
const EMPTY = '';

describe('lambda-layer-git', () => {
  var targetDirectory;
  ...
  it('Should return a promise', () => {
    return expect(LambdaLayerGit({ targetDirectory })).to.be.instanceof(Promise);
  });

  it('Should not change the env, if explicitly asked', () => {
    process.env.GIT_TEMPLATE_DIR = EMPTY;
    process.env.GIT_EXEC_PATH = EMPTY;
    return LambdaLayerGit({ targetDirectory, updateEnv: false }).then(() => {
  ...
  it('Should return a promise, if not updating the env', () => {
    return expect(LambdaLayerGit({ targetDirectory, updateEnv: false })).to.be.instanceof(Promise);
  });

  it('rejects promise if fs.existsSync() throws', () => {
    return expect(LambdaLayerGit({ targetDirectory, updateEnv: false })).to.eventually.eql({
